...
class StatisticsMinimalAlgorithm[A <: StatisticsAggregator[A] : ClassTag, T <: StatisticsMinimalAlgorithmObject[T, A] : ClassTag]
  ...
  /**
    ...
    */
  def computePrefix: RDD[(A, T)] = computePrefix(this.objects)
  ...
  /**
    ...
    */
  def computePrefix(rdd: RDD[T]): RDD[(A, T)] = {
  ...
  def getPrefixedPartitionStatistics(rdd: RDD[T]): Array[A] = {
  ...
  /**
    ...
    */
  def getPartitionsStatistics(rdd: RDD[T]): RDD[A] = {
