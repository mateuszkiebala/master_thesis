...
class MinimalSlidingAggregation(spark: SparkSession, numPartitions: Int) extends MinimalAlgorithm(spark, numPartitions) {
  ...
  (implicit ord: Ordering[K], ttag: ClassTag[T], ktag: ClassTag[K], stag: ClassTag[S]): RDD[(T, S)] = {
    val (itemsCntByPartition, rddItemsCount) = computeItemsCntByPartition(rdd, itemsCnt)
    val rankedData = perfectSortWithRanks(rdd, cmpKey, rddItemsCount).persist()
    val distData = distributeDataToRemotelyRelevantPartitions(rankedData, windowLength, itemsCntByPartition).persist()
    ...
    windowValues(distData, windowLength, itemsCntByPartition, partitionsRangeTree, statsAgg)
  }

  private[this] def distributeDataToRemotelyRelevantPartitions[T]
  (rdd: RDD[(Int, T)], windowLength: Int, itemsCntByPartition: Int)(implicit ttag: ClassTag[T]): RDD[(Int, T)] = {
    ...
    Utils.sendToMachines(rdd.mapPartitionsWithIndex((pIndex, partitionIt) => {
      if (windowLength <= itemsCntByPartition) {
        ...
        val remRelM = (windowLength-1) / itemsCntByPartition
        partitionIt.map{rankPair =>
          val machineIndices =
  ...
  (rdd: RDD[(Int, T)], windowLength: Int, itemsCntByPartition: Int, partitionsRangeTree: RangeTree[S], statsAgg: T => S)
  (implicit ttag: ClassTag[T], stag: ClassTag[S]): RDD[(T, S)] = {
    rdd.mapPartitionsWithIndex((pIndex, partitionIt) => {
      val baseLowerBound = pIndex * itemsCntByPartition
      val baseUpperBound = (pIndex+1) * itemsCntByPartition - 1
      ...
      baseObjects.map{case (rank, o) => {
        ...
        val a = ((rank+1-windowLength+itemsCntByPartition) / itemsCntByPartition) - 1
        val alpha = if (a >= 0) a else -1
        val result = if (pIndex > alpha + 1) {
          val w1 = if (alpha < 0) null.asInstanceOf[S] else rangeTree.query(rankToIndex(windowStart), rankToIndex((alpha+1)*itemsCntByPartition-1))
