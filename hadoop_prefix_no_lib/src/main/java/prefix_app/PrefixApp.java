...
public class PrefixApp extends Configured implements Tool {
  ...
  public int run(String[] args) throws Exception {
    if (args.length != 6) {
      System.err.println("Usage: PrefixApp <home_dir> <input_path> <output_path>" +
    ...
    PartStats.run(new Path(args[0] + "/sorting"), new Path("part_stats"), conf);
    return Prefix.run(new Path(args[0] + "/sorting"),
