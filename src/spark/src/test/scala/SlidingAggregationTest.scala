...
class SlidingAggregationTest extends FunSuite with SharedSparkContext with Matchers {
  ...
  test("SlidingAggregation sum") {
    ...
    val result = new MinimalAlgFactory(spark, 3, createRDD(spark)).slidingAggregation(7, cmpKey, statsAgg).collect()
  ...
  test("SlidingAggregation min") {
    ...
    val result = new MinimalAlgFactory(spark, 3, createRDD(spark), 11).slidingAggregation(7, cmpKey, statsAgg).collect()
