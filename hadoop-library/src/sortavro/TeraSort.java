...
public class TeraSort extends Configured implements Tool {
    ...
    public int run(String[] args) throws Exception {
        ...
        int noOfInputValues = Integer.parseInt(args[2]);// n no of inpput data values
        int noOfStrips = Integer.parseInt(args[3]);// t no of strips
        int noOfValuesPerStrip = 1 + noOfInputValues / noOfStrips;//m no of data values per strip
        if (noOfValuesPerStrip / 20 > Integer.MAX_VALUE) {
            System.err.println("Too many values for one strip. Increase number of strips to avoid int overflow.");
            System.exit(-1);
        }
        double rho = 1. / noOfValuesPerStrip * Math.log(((double) noOfInputValues) * noOfStrips);
        int reverseRho = (int) (1 / rho);
        System.out.println("noOfInputValues = " + noOfInputValues);
        System.out.println("noOfStrips = " + noOfStrips);
        System.out.println("noOfValuesPerStrip = " + noOfValuesPerStrip);
        System.out.println("rho="+rho);
        System.out.println("reverseRho="+reverseRho);
        System.out.println("for smaling keys aprox. = " + (int) (noOfInputValues * rho));

        Configuration conf = getConf();
        conf.setLong(PhaseSampling.NO_OF_VALUES_KEY, noOfInputValues);
        conf.setInt(PhaseSampling.NO_OF_STRIPS_KEY, noOfStrips);
    ...
    public static void main(String[] args) throws Exception {
        int res = ToolRunner.run(new Configuration(), new TeraSort(), args);//to configuration jest tu potrzebne zeby odczytac -libjars (http://stackoverflow.com/questions/28520821/how-to-add-external-jar-to-hadoop-job)
        System.exit(res);
    }
}
